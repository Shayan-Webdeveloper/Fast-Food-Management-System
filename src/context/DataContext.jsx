...
const mapOrder = (order) => ({
  ...
  deliveryName: order.delivery_name, deliveryPhone: order.delivery_phone, deliveryAddress: order.delivery_address, orderType: order.order_type,
})
...
const [customerProfiles, setCustomerProfiles] = useState([])
  ...
  const refreshData = useCallback(async () => {
    ...
    const [orderResult, notificationResult, profileResult] = await Promise.all([
      ...
      supabase.from('notifications').select('*, orders(order_number, total, payment_method, delivery_name, delivery_phone, delivery_address, order_type, profiles!orders_customer_id_fkey(full_name, email), order_items(quantity, unit_price, menu_items(name)))').order('created_at', { ascending: false }),
...
useEffect(() => {
    ...
    const voidOrder = useCallback(async (orderNumber, reason) => {
      const order = orders.find((item) => item.id === orderNumber)
      if (!order) return
      const { error } = await supabase.rpc('void_counter_order', { p_order_id: order.dbId, p_reason: reason || null })
      if (error) throw error
      await refreshMenu()
      setOrders((current) => current.map((item) => item.id === orderNumber ? { ...item, status: 'cancelled' } : item))
    }, [orders, refreshMenu])
...
const placeOrder = useCallback(async (items, paymentMethod = 'card', delivery = {}, orderType = 'delivery', discount = {}) => {
    ...
    return <DataContext.Provider value={{ menu, orders, allOrders: orders, notifications, cart, cartTotal, loading, customerProfiles, refreshMenu, addMenuItem, updateMenuItem, deleteMenuItem, updateOrderStatus, submitRating, markReturned, voidOrder, placeOrder, addToCart, removeFromCart, updateCartQty, clearCart: () => setCart([]), markNotificationRead, markAllNotificationsRead }}>{children}</DataContext.Provider>
  }
